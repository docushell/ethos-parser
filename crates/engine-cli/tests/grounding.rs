...
//!
//! This lives in `engine-cli` because it is the only crate that depends on both `engine-pdf`
//! (which produces a representation from a PDF) and `engine-grounding` (which projects one).
...
//!
//! The schema-subset validator is shared with `engine-grounding`'s own tests by path include,
...
use engine_core::{DocumentRepresentation, GeometryPresence, Profile};
use engine_grounding::GroundingSource;
use serde_json::Value;

#[path = "../../engine-grounding/tests/schema_subset.rs"]
mod schema_subset;
...
fn represent(path: &Path) -> DocumentRepresentation {
    ...
    engine_pdf::to_representation(&extract, &profile).expect("represents")
}

fn ground(path: &Path) -> engine_grounding::Projection {
    engine_grounding::project(&represent(path)).expect("projects")
}
...
fn as_value(g: &GroundingSource) -> Value {
    serde_json::from_slice(&engine_grounding::to_canonical_bytes(g).expect("canonical"))
        .expect("valid JSON")
}
...
fn every_emitted_box_is_ordered_and_inside_its_page() {
    ...
    for name in ["measured-ink-box"] {
        ...
        let page_by_id: std::collections::BTreeMap<&str, &engine_grounding::Page> =
...
fn a_node_without_measurable_geometry_is_kept_counted_and_declared() {
    ...
    match &node.native_locator {
        engine_core::NativeLocator::Pdf(l) => {
            assert_eq!(l.page, 1);
            assert_eq!(
                l.advance,
                Some(16800),
    ...
    let p = engine_grounding::project(&repr).unwrap();
...
fn omission_selects_rather_than_empties() {
    ...
    let p = engine_grounding::project(&repr).expect("projects");
...
fn every_emitted_box_is_the_measured_box() {
    ...
    for path in [
        ...
        let repr = represent(&path);
        let g = engine_grounding::project(&repr).expect("projects").source;
...
fn the_omission_call_site_takes_a_measurement_state_and_nothing_else() {
    use engine_core::{GeometryAbsence, QRect};
    use engine_grounding::GroundedBox;
    ...
    let code = crate_code("engine-grounding");
    assert!(
...
fn representation_and_grounding_are_byte_identical_across_runs() {
    for id in OPENABLE {
        ...
        let g1 = engine_grounding::to_canonical_bytes(&ground(&path).source).unwrap();
        let g2 = engine_grounding::to_canonical_bytes(&ground(&path).source).unwrap();
        assert_eq!(g1, g2, "{id}: grounding is not byte-identical");
    }
}

/// `engine-grounding` contains no PDF concept, and never reads a locator.
#[test]
fn engine_grounding_has_no_pdf_concept() {
    let code = crate_code("engine-grounding");
    for banned in [
        "lopdf",
        "engine_pdf",
        ...
        assert!(
            ...
            "`{banned}` appears in engine-grounding code"
        );
    }
    ...
    for banned in ["NativeLocator", "PdfLocator", "StructuralLocator"] {
        assert!(
            !code.contains(banned),
            "engine-grounding reads `{banned}`; the projection addresses pages by node id \
             precisely so it never has to"
        );
    }

    let manifest =
        std::fs::read_to_string(repo_root().join("crates/engine-grounding/Cargo.toml")).unwrap();
    assert!(
        !manifest.contains("engine-pdf"),
        "engine-grounding must not depend on engine-pdf"
    );
...
fn the_artifact_satisfies_the_invariants_the_schema_cannot_express() {
        ...
        )
    {
        ...
        let bytes = engine_grounding::to_canonical_bytes(&g).unwrap();
...
fn no_confidence_in_a_representation_or_a_grounding_artifact() {
    ...
    for (label, text) in [
        ...
        (
            "grounding",
            String::from_utf8(engine_grounding::to_canonical_bytes(&g).unwrap()).unwrap(),
        ),
    ] {
...
fn the_cli_path_matches_the_library() {
    ...
    let extract_out = Command::new(env!("CARGO_BIN_EXE_engine"))
        .arg("extract")
        .arg(&pdf)
        .output()
        .expect("runs");
    ...
    assert_eq!(
        ...
        "`engine extract` must emit exactly the library's representation bytes"
    );

    // Round-trip through a file, exactly as a caller would.
    let dir = std::env::temp_dir().join(format!("ethos-engine-m5-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let repr_path = dir.join("repr.json");
    std::fs::write(&repr_path, &extract_out.stdout).unwrap();

    let ground_out = Command::new(env!("CARGO_BIN_EXE_engine"))
        .arg("ground")
        .arg(&repr_path)
        .output()
        .expect("runs");
    ...
    let expected_ground = engine_grounding::to_canonical_bytes(&ground(&pdf).source).unwrap();
    assert_eq!(
...
fn ground_refuses_a_representation_whose_fingerprint_disagrees() {
    ...
    let dir = std::env::temp_dir().join(format!("ethos-engine-m5-tamper-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn the_omission_report_goes_to_stderr_so_stdout_stays_byte_identical() {
    let out = Command::new(env!("CARGO_BIN_EXE_engine"))
        ...
        .output()
        .expect("runs");
    let dir = std::env::temp_dir().join(format!("ethos-engine-m5-omit-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let p = dir.join("repr.json");
    std::fs::write(&p, &out.stdout).unwrap();

    let g = Command::new(env!("CARGO_BIN_EXE_engine"))
        .arg("ground")
        .arg(&p)
        .output()
        .expect("runs");
...
fn a_budgeted_run_still_produces_a_coherent_record_and_projection() {
    let profile = Profile {
        page_budget: engine_core::PageBudget::AtMost(1),
        ..Profile::default()
    };
    let path = {
        let m = manifest();
    ...
    let doc = engine_pdf::Document::open(&path, &profile).expect("opens");
    let extract = engine_pdf::extract(&doc, &profile).expect("extracts");
    let repr = engine_pdf::to_representation(&extract, &profile).expect("represents");
