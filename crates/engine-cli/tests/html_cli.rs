...
//! `engine html` — the second projection, under the same four laws (v1.1-S4).
//!
...
fn html_on_simple_text_is_the_artifact_the_scope_document_describes() {
    ...
    assert_eq!(a["html_rule"], "html-blocks-v1");
...
fn no_cli_path_emits_html_without_its_map() {
    ...
    for flag in ["--html-only", "--no-map", "--raw", "--bare"] {
        assert!(
            ...
            "`{flag}` exists on `engine html`, and law 1 says the map is not optional"
        );
    }
