...
//! process. `CARGO_BIN_EXE_engine` does not appear, and
...
//!
//! # Why it lives in `engine-cli`
//!
//! Because it is the only crate that depends on all three libraries. `engine-grounding` may never
//! depend on `engine-pdf` (`docs/04-ARCHITECTURE.md` §1), so a test spanning extract → project
//! has nowhere else to go. The crate is the host; the binary is not involved.

use std::path::PathBuf;

use engine_core::{DocumentRepresentation, Profile};
...
/// `engine classify` — reachable as `Document::open` → `classify` → `to_canonical_bytes`.
#[test]
fn classify_is_reachable_and_canonical_from_the_library() {
    ...
    let classification = engine_pdf::classify(&doc, &profile).expect("classifies");
    ...
    assert_eq!(
        ...
        "ethos.engine.classification.v0"
    );
    ...
    let code = engine_pdf::exit::exit_code(&Ok(classification));
    assert!(
        code == engine_pdf::exit::SIMPLE || code == engine_pdf::exit::NEEDS_ATTENTION,
        "a document that classified maps to 0 or 1, never {code}"
    );
}

/// `engine extract` — reachable, and it is the **representation** the subcommand emits.
#[test]
fn extract_and_represent_are_reachable_and_canonical_from_the_library() {
    ...
    let v = parse_canonical(&bytes);
    assert_eq!(v["artifact_type"], "ethos.engine.representation.v0");
    assert!(
...
/// `engine ground` — reachable as parse → `verify_fingerprint` → `project` → canonical bytes.
///
...
fn project_is_reachable_and_canonical_from_the_library() {
    ...
    let projection = engine_grounding::project(&reparsed).expect("projects");
    let bytes = engine_grounding::to_canonical_bytes(&projection.source).expect("canonicalizes");
    let v = parse_canonical(&bytes);
...
/// `engine grounding-check` — reachable, including the report's own exit-code mapping.
#[test]
fn grounding_check_is_reachable_and_canonical_from_the_library() {
    ...
    let extract = engine_pdf::extract(&doc, &profile).expect("extracts");
    let repr = engine_pdf::to_representation(&extract, &profile).expect("represents");
    let projection = engine_grounding::project(&repr).expect("projects");
    let grounding =
        engine_grounding::to_canonical_bytes(&projection.source).expect("canonicalizes");

    // With the source: the binding is answered.
    let matched = engine_grounding::grounding_check(&grounding, Some(&source)).expect("checks");
    ...
    let unchecked = engine_grounding::grounding_check(&grounding, None).expect("checks");
    ...
    let mismatched = engine_grounding::grounding_check(&grounding, Some(&other)).expect("checks");
...
fn the_whole_happy_path_runs_inside_one_process() {
    ...
    let _classification = engine_pdf::classify(&doc, &profile).expect("classifies");
    let extract = engine_pdf::extract(&doc, &profile).expect("extracts");
    let repr = engine_pdf::to_representation(&extract, &profile).expect("represents");
    let projection = engine_grounding::project(&repr).expect("projects");
    let grounding =
        engine_grounding::to_canonical_bytes(&projection.source).expect("canonicalizes");
    let report = engine_grounding::grounding_check(&grounding, Some(&source)).expect("checks");
    ...
    assert_eq!(
        ...
        engine_core::sha256_hex_bytes(&source)
    );
}
...
fn every_geometry_bearing_artifact_declares_its_coordinate_system() {
    ...
    let g = parse_canonical(
        &engine_grounding::to_canonical_bytes(&projection.source).expect("canonical"),
    );
...
fn the_library_refuses_what_the_binary_refuses() {
    ...
    assert_eq!(
        engine_pdf::exit::exit_code(&Err(e)),
        engine_pdf::exit::COULD_NOT_READ,
...
///
/// The whole claim is "without the CLI". A `Command::new(env!("CARGO_BIN_EXE_engine"))` added
