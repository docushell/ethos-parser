...
/// **`engine classify` did NOT move with `engine extract`, and that is stated rather than left
/// to be discovered.**
///
...
/// looked equivalent and was not: `engine-pdf/src/lib.rs` carries a `#[cfg(test)]` at line 56, so
