...
//! `engine extract` reads an ODT, and every path that would have needed a page refuses (v2-S5).
//!
//! The fourth sibling of `office_cli.rs`, proving the same claim over a container that is not
//! OOXML: **one subcommand, one artifact type, one serializer**. There is no
//! `ethos.engine.odt.v0` and no `engine extract-odt`; dispatch is by content; and `node_get`
//! resolves an ODF paragraph over an MCP server that was never taught what OpenDocument is.
...
#[test]
fn extract_reads_an_odt_into_the_same_artifact_type_a_pdf_produces() {
    ...
    assert!(
        artifact["representation"]["pages"]
            ...
            .expect("pages")
            .is_empty(),
        "the producer's own page break is not a page this engine read, so there are none"
    );
...
fn a_renamed_document_still_reads_and_a_renamed_pdf_does_not_become_one() {
    ...
    let liar = dir.join("not-really.odt");
...
#[test]
fn two_runs_over_one_document_produce_identical_bytes() {
...
/// **(b) still holds.** `ethos.grounding.v1` is PDF-only, and an ODT is refused by name.
///
/// This passes with **no change to `engine-grounding`**: `project` refuses on the source media
/// type, which an ODT artifact fails for the same reason a DOCX one does.
#[test]
fn ground_projects_a_page_less_text_document_by_name() {
    let dir = tempdir();
...
/// An ODF paragraph resolves through the same fingerprint-checked handle path a PDF run uses, and
...
fn node_get_resolves_a_paragraph_and_fails_closed_on_a_forged_id() {
    ...
    let locator = &ok["structuredContent"]["native_locator"]["odt"];
    assert_eq!(
    ...
    assert!(
        locator.get("page").is_none()
            ...
            && locator.get("soft_page_break").is_none(),
        "and it carries no page, no box and no page break: {locator:?}"
    );
