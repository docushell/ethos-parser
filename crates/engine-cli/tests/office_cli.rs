...
//! `engine extract` reads a DOCX, and every path that would have needed a page refuses (v2-S2).
//!
//! **One subcommand, one artifact type** (`docs/14-V2-SCOPE.md` §4). There is no
//! `ethos.engine.docx.v0` and no `engine extract-docx`: dispatch is by content, the record is the
