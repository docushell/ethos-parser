...
//! `engine extract` reads a PPTX, and every path that would have needed a page refuses (v2-S3).
//!
...
//! `ethos.engine.pptx.v0` and no `engine extract-pptx`; dispatch is by content; and `node_get`
...
#[test]
fn ground_refuses_a_page_less_deck_by_name() {
    let dir = tempdir();
