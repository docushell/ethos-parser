...
/// **EPUB and CSV are still S9**, and neither is told it is something it is not.
///
/// The one thing v2-S6 pinned that this slice had to leave alone: an `.epub` uses OCF's
/// first-and-stored `mimetype` entry, the same container rule every ODF package follows, and it
/// must not be answered "this is an OpenDocument package". A `.csv` cannot be told from prose
/// without a reader, so it takes the true-unknown-bytes path — recorded in
/// `docs/15-V2-MILESTONES.md` S9 rather than guessed at here.
#[test]
fn the_formats_this_slice_did_not_implement_are_still_refused() {
    let dir = tempdir();

    let epub = dir.join("book.epub");
    std::fs::write(&epub, ocf_package("application/epub+zip")).expect("write the package");
    let (code, stdout, stderr) = extract(&epub);
    assert_eq!(code, 2, "an EPUB has no reader yet: {stderr}");
    assert!(stdout.is_empty());
    assert!(
        !stderr.contains("this is an OpenDocument package"),
        "and it is NOT told it IS OpenDocument, which v2-S6 pinned — the message may list the ODF \
         types it is not, which is a negative rather than a claim: {stderr}"
    );
    assert!(
        !stderr.contains("%PDF-"),
        "nor that it is a PDF missing its header: a ZIP is definitively not one, so the refusal \
         names the container it actually is: {stderr}"
    );
    assert!(
        stderr.contains("ZIP") || stderr.contains("package"),
        "the refusal names the container shape: {stderr}"
    );
