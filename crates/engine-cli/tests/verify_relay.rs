...
//! `engine verify` — relaying, not verifying (`docs/07-VERIFY-BOUNDARY.md` Stage 1).
//!
...
//! 3. **The bytes are the verifier's.** `engine verify` stdout is byte-identical to running
...
fn build(dir: &Path) -> Case {
    ...
    let fingerprint = format!("sha256:{}", engine_core::sha256_hex_bytes(&bytes));
...
fn the_relay_is_reachable_from_the_library() {
    ...
    assert_eq!(relayed.exit, engine_core::RELAY_OK);
...
fn pinning_a_different_verifier_moves_the_profile_hash() {
    use engine_core::{Profile, Sha256Hex, VerifierPin};
