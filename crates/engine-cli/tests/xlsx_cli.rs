...
//! `engine extract` reads an XLSX, and every path that would have needed a page refuses (v2-S3).
//!
...
//! `ethos.engine.xlsx.v0` and no `engine extract-xlsx`; dispatch is by content; and `node_get`
//! resolves a *cell* over an MCP server that was never taught what a workbook is.
...
fn fixture() -> PathBuf {
    repo_root().join("fixtures/office/workbook-cells/workbook.xlsx")
}
...
#[test]
fn extract_reads_an_xlsx_into_the_same_artifact_type_a_pdf_produces() {
    let (code, stdout, stderr) = extract(&fixture());
    assert_eq!(code, 0, "{stderr}");

    let artifact: Value = serde_json::from_slice(&stdout).expect("canonical JSON");
    assert_eq!(artifact["artifact_type"], "ethos.engine.representation.v0");
    assert!(
        artifact["representation"]["pages"]
            .as_array()
            .expect("pages")
            .is_empty(),
        "a workbook's pages are a print artefact, so there are none to declare"
    );
...
#[test]
fn a_renamed_workbook_still_reads_and_a_renamed_pdf_does_not_become_one() {
    let dir = tempdir();
    ...
    let liar = dir.join("not-really.xlsx");
...
#[test]
fn two_runs_over_one_xlsx_produce_identical_bytes() {
...
/// **(b) still holds.** `ethos.grounding.v1` is PDF-only, and a workbook is refused by name.
///
/// This passes with **no change to `engine-grounding`**: `project` refuses on the source media
/// type, which an XLSX artifact fails for the same reason a DOCX one does.
#[test]
fn ground_projects_a_page_less_workbook_by_name() {
    let dir = tempdir();
...
/// A cell resolves through the same fingerprint-checked handle path a PDF run uses, and a forged
...
fn node_get_resolves_a_cell_and_fails_closed_on_a_forged_id() {
    ...
    let locator = &ok["structuredContent"]["native_locator"]["xlsx"];
    assert_eq!(
        locator["sheet"], "Ledger",
        "the address that came back is the workbook's own: {locator:?}"
    );
    assert_eq!(locator["part"], "xl/worksheets/sheet1.xml");
    assert_eq!(locator["row"], json!(1));
    assert_eq!(locator["column"], "A");
    assert!(
