...
//!
//! `engine-core`'s dev-dependencies enable that feature so this test runs in exactly that
...
fn assert_preserve_order_is_actually_enabled() {
    ...
    assert_eq!(
        ...
        "serde_json emitted sorted order, so `preserve_order` is NOT enabled for this build. \
         ...
         in crates/engine-core/Cargo.toml rather than deleting the assertion."
    );
}
...
fn the_committed_parity_hashes_still_hold_under_preserve_order() {
    ...
    assert_eq!(
        engine_core::sha256_hex(&Value::Object(m)).unwrap(),
...
fn the_profile_hash_is_unaffected_by_map_flavour() {
    ...
    let a = engine_core::Profile::default().profile_sha256().unwrap();
    let b = engine_core::Profile::default().profile_sha256().unwrap();
    assert_eq!(a, b);

    // Round-tripping through a parsed (insertion-ordered) value must not move it either.
    let bytes = engine_core::Profile::default().canonical_bytes().unwrap();
