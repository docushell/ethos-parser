...
//! exists to refuse. `pages` is `[]`, and [`engine_core::OdtLocator`] has no room for one.
//!
//! # What a paragraph's text is, and what it is not
//!
//! The atom is the `<text:p>` or `<text:h>` block; see [`engine_core::OdtLocator`] for why it is
