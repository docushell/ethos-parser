...
use engine_core::{GeometryPresence, NativeLocator, Profile};
...
fn a_quote_resolves_to_a_node_addressed_by_the_document_itself() {
    let sealed = engine_office::read(&fixture("simple-paragraphs")).expect("the fixture reads");
...
fn the_artifact_has_no_pages_no_geometry_and_a_part_for_a_parent() {
    let sealed = engine_office::read(&fixture("simple-paragraphs")).expect("reads");
    let payload = sealed.payload();
...
fn the_artifact_declares_its_own_profile_and_its_own_media_type() {
    ...
    assert_eq!(payload.source.media_type, engine_office::DOCX_MEDIA_TYPE);
    assert_eq!(
...
fn parts_this_slice_does_not_read_are_declared_with_a_count() {
    let sealed = engine_office::read(&fixture("unread-parts")).expect("reads");
...
fn detection_reads_the_bytes() {
    ...
    assert!(engine_office::read(&docx).is_ok());
    ...
    for not_a_docx in [
        ...
        assert!(
            !engine_office::is_docx(not_a_docx),
...
fn a_zip_without_the_main_part_is_a_named_refusal() {
    ...
    let error = engine_office::read(&bytes).expect_err("refused");
    assert!(
