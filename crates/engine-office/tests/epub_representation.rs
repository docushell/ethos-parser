...
fn entry_of(bytes: &[u8], name: &str) -> String {
    let part = engine_office::zip::read_entry(bytes, name)
        .unwrap_or_else(|e| panic!("`{name}` inflates: {e}"));
    String::from_utf8(part).expect("the entry is UTF-8")
}

fn locators(sealed: &engine_core::DocumentRepresentation) -> Vec<EpubLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn text_at(sealed: &engine_core::DocumentRepresentation, part: &str, block: u32) -> Option<String> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn all_text(sealed: &engine_core::DocumentRepresentation) -> String {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_block_resolves_to_a_node_addressed_by_the_package_itself() {
    let sealed = engine_office::read(&fixture("book-spine")).expect("the fixture reads");
...
fn the_spine_states_the_order_and_the_archive_does_not() {
    ...
    let names = engine_office::zip::entry_names(&bytes).expect("the archive lists its entries");
...
fn removing_a_region_from_the_bytes_moves_the_declared_count() {
    ...
    assert!(
        !all_text(&engine_office::read(&mutated).expect("the mutated package reads"))
...
fn the_clean_publication_declares_no_erasure_and_the_other_declares_every_kind() {
    let clean = engine_office::read(&fixture("book-spine")).expect("reads");
    assert!(
        !clean
            .payload()
            .assurance
    ...
    let detail = a14_detail(&engine_office::read(&fixture("book-unread-parts")).expect("reads"));
...
fn an_encrypted_spine_document_is_refused_and_an_obfuscated_font_is_not() {
    ...
    let sealed = engine_office::read(&font_only).expect("a clear-text publication still reads");
    ...
    let error = engine_office::read(&content_encrypted).expect_err("refused");
...
fn a_publication_with_no_container_is_a_named_refusal() {
    ...
    let error = engine_office::read(&broken).expect_err("refused");
    let text = error.to_string();
...
fn a_duplicated_entry_is_refused() {
    let bytes = fixture("book-spine");
    let names = engine_office::zip::entry_names(&bytes).expect("entries");
    let mut owned: Vec<(String, Vec<u8>)> = names
        .iter()
        .map(|n| {
            (
                n.clone(),
                engine_office::zip::read_entry(&bytes, n).expect("inflates"),
            )
        })
        .collect();
    ...
    let error = engine_office::read(&build_zip(&borrowed)).expect_err("refused");
...
fn the_epub_profile_is_its_own_and_all_nine_are_distinct() {
    ...
    assert_eq!(
        ...
        engine_core::EPUB_READING_ORDER_RULE_V1
    );
}
...
fn detection_reads_the_bytes_and_an_epub_is_never_opendocument() {
    let bytes = fixture("book-spine");
    assert!(engine_office::is_epub(&bytes));
    assert!(
        !engine_office::is_opendocument(&bytes),
    ...
    for (kind, claimed) in [
        ("odt", engine_office::is_odt(&bytes)),
        ("ods", engine_office::is_ods(&bytes)),
        ("odp", engine_office::is_odp(&bytes)),
        ("docx", engine_office::is_docx(&bytes)),
        ("xlsx", engine_office::is_xlsx(&bytes)),
        ("pptx", engine_office::is_pptx(&bytes)),
        ("rtf", engine_office::is_rtf(&bytes)),
    ] {
    ...
    assert!(!engine_office::is_epub(&odt));
...
fn a_near_miss_declaration_is_not_claimed() {
    ...
    assert!(!engine_office::is_epub(&reordered));
}
...
fn a14_detail(sealed: &engine_core::DocumentRepresentation) -> String {
    sealed
        .payload()
        .assurance
        .limitations
...
fn with_entry(original: &[u8], name: &str, body: &str) -> Vec<u8> {
    let names = engine_office::zip::entry_names(original).expect("the archive lists its entries");
    ...
    for existing in &names {
        let raw = if existing == name {
            ...
            engine_office::zip::read_entry(original, existing).expect("inflates")
        };
