...
fn locators(sealed: &engine_core::DocumentRepresentation) -> Vec<OdpLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn text_at(
    sealed: &engine_core::DocumentRepresentation,
    draw_page: u32,
    shape: u32,
    paragraph: u32,
) -> Option<String> {
...
fn a_title_resolves_to_a_node_addressed_by_the_document_itself() {
    let sealed = engine_office::read(&fixture("presentation-pages")).expect("the fixture reads");
...
fn a_draw_page_is_listed_in_the_file_and_is_still_not_a_page() {
    ...
    let styles = String::from_utf8(
        engine_office::zip::read_entry(&fixture("presentation-unread-parts"), "styles.xml")
...
fn non_displayed_character_data_is_absent_from_every_block_and_is_declared() {
    ...
    let styles = String::from_utf8(
        engine_office::zip::read_entry(&bytes, "styles.xml").expect("styles inflates"),
    )
    .expect("utf-8");
...
fn removing_the_speaker_notes_from_the_bytes_moves_the_declared_count() {
    ...
    let sealed = engine_office::read(&mutated).expect("the mutated package reads");
    assert_eq!(
...
fn the_clean_package_declares_no_erasure_and_the_other_declares_three_kinds() {
    let clean = engine_office::read(&fixture("presentation-pages")).expect("reads");
    assert!(
        !clean
            .payload()
            .assurance
    ...
    let dirty = engine_office::read(&fixture("presentation-unread-parts")).expect("reads");
    let detail = dirty
        .payload()
        .assurance
        .limitations
...
fn detection_reads_the_bytes() {
    let bytes = fixture("presentation-pages");
    assert!(engine_office::is_odp(&bytes));
    ...
    for (path, kind) in [
        ...
        assert!(
            !engine_office::is_odp(&sibling),
            "an {kind} is not a presentation"
        );
        assert!(engine_office::is_opendocument(&sibling));
    }
}
...
fn a_template_and_a_drawing_are_not_claimed_as_a_presentation() {
    for (declared, kind) in [
        ...
        let error = engine_office::read(&archive).expect_err("and none of them reads");
        ...
        if kind == "epub" {
            assert!(
                !engine_office::is_opendocument(&archive),
