...
fn locators(sealed: &engine_core::DocumentRepresentation) -> Vec<PptxLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_slide_run_resolves_to_a_node_addressed_by_the_deck_itself() {
    let sealed = engine_office::read(&fixture("deck-slides")).expect("the fixture reads");
...
fn the_artifact_declares_its_own_profile_and_its_own_media_type() {
    ...
    assert_eq!(payload.source.media_type, engine_office::PPTX_MEDIA_TYPE);
    assert_eq!(
...
fn parts_and_shapes_this_slice_does_not_read_are_declared_with_counts() {
    let sealed = engine_office::read(&fixture("deck-unread-parts")).expect("the fixture reads");
...
fn an_unread_shape_alone_is_enough_to_declare() {
    ...
    let sealed = engine_office::read(&archive).expect("the deck reads");
...
fn detection_reads_the_bytes() {
    ...
    assert!(engine_office::is_pptx(&pptx));
    assert!(!engine_office::is_docx(&pptx));
    assert!(!engine_office::is_xlsx(&pptx));
    assert!(engine_office::read(&pptx).is_ok());
    ...
    for not_a_deck in [
        ...
        assert!(!engine_office::is_pptx(not_a_deck));
    }
}
...
fn a_package_that_is_two_formats_is_refused_by_name() {
    ...
    assert!(engine_office::is_pptx(&archive));
...
fn a_zip_without_a_presentation_part_is_a_named_refusal() {
    ...
    assert!(!engine_office::is_pptx(&renamed));
