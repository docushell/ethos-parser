...
fn locators(sealed: &engine_core::DocumentRepresentation) -> Vec<RtfLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn text_at(sealed: &engine_core::DocumentRepresentation, paragraph: u32) -> Option<String> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_paragraph_resolves_to_a_node_addressed_by_the_stream_itself() {
    let sealed = engine_office::read(&fixture("rich-text-paragraphs")).expect("the fixture reads");
...
fn removing_a_destination_from_the_bytes_moves_the_declared_count() {
    ...
    let sealed = engine_office::read(stripped.as_bytes()).expect("the mutated stream reads");
    assert_eq!(
...
fn the_clean_stream_declares_no_erasure_and_the_other_declares_two_kinds() {
    let clean = engine_office::read(&fixture("rich-text-paragraphs")).expect("reads");
    assert!(
        !clean
            .payload()
            .assurance
    ...
    let detail =
        a14_detail(&engine_office::read(&fixture("rich-text-unread-destinations")).expect("reads"));
...
fn detection_reads_the_bytes_and_refuses_the_near_misses() {
    let bytes = fixture("rich-text-paragraphs");
    assert!(engine_office::is_rtf(&bytes));
    assert!(!engine_office::is_docx(&bytes));
    assert!(!engine_office::is_xlsx(&bytes));
...
fn bytes_that_are_neither_are_refused_naming_both_shapes() {
    let error = engine_office::read(b"plain text, no container").expect_err("refused");
    let text = error.to_string();
    assert!(
        text.contains(r"{\rtf"),
