...
fn locators(sealed: &engine_core::DocumentRepresentation) -> Vec<XlsxLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn cell_at<'a>(
    sealed: &'a engine_core::DocumentRepresentation,
    sheet: &str,
    column: &str,
    row: u32,
) -> Option<&'a engine_core::Node> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_cell_resolves_to_a_node_addressed_by_the_workbook_itself() {
    let sealed = engine_office::read(&fixture("workbook-cells")).expect("the fixture reads");
...
fn the_artifact_declares_its_own_profile_and_its_own_media_type() {
    ...
    assert_eq!(payload.source.media_type, engine_office::XLSX_MEDIA_TYPE);
    assert_eq!(
...
fn parts_this_slice_does_not_read_are_declared_with_a_count() {
    let sealed = engine_office::read(&fixture("workbook-unread-parts")).expect("the fixture reads");
...
fn a_listed_sheet_that_is_not_a_worksheet_is_declared_with_a_count() {
    ...
    let sealed = engine_office::read(&archive).expect("the workbook reads");
    let limitation = sealed
        .payload()
        .assurance
        .limitations
...
fn detection_reads_the_bytes() {
    ...
    assert!(engine_office::is_xlsx(&xlsx));
    assert!(
        !engine_office::is_docx(&xlsx),
        "a workbook lists no `word/document.xml`, so it is never claimed as a document"
    );
    assert!(engine_office::read(&xlsx).is_ok());
    ...
    assert!(
        !engine_office::is_xlsx(&docx),
        "and a document is never claimed as a workbook"
    );
    assert!(engine_office::is_docx(&docx));
    ...
    for not_a_workbook in [
        ...
        assert!(!engine_office::is_xlsx(not_a_workbook));
    }
}
...
fn a_package_that_is_both_formats_is_refused_by_name() {
    ...
    assert!(engine_office::is_xlsx(&archive));
...
fn a_zip_without_a_workbook_part_is_a_named_refusal() {
    ...
    assert!(!engine_office::is_xlsx(&renamed));
    assert!(!engine_office::is_docx(&renamed));

    let error = engine_office::read(&renamed).expect_err("a ZIP that is neither format");
    let text = error.to_string();
    assert!(text.contains("word/document.xml"), "{text}");
    assert!(text.contains("xl/workbook.xml"), "{text}");
}
