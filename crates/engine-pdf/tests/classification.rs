...
use engine_pdf::exit::{exit_code, COULD_NOT_READ, NEEDS_ATTENTION, SIMPLE};
use engine_pdf::{Classification, Document, LayoutComplexityReason, OcrNeedReason};
...
fn run(path: PathBuf, profile: &Profile) -> Result<Classification, EngineError> {
    Document::open(&path, profile).and_then(|d| engine_pdf::classify(&d, profile))
}
...
fn unknown_magic_fails_closed() {
    let dir = std::env::temp_dir().join("ethos-engine-m2-magic");
...
fn the_artifact_carries_a_full_identity_envelope() {
    ...
    assert_eq!(
        ...
        engine_pdf::CLASSIFICATION_ARTIFACT_TYPE
    );
    assert_eq!(
        c.identity.schema_version,
        engine_pdf::CLASSIFICATION_SCHEMA_VERSION
    );
