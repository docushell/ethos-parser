...
///
/// **A stage is not a subcommand**, and `Stage`'s own doc comment says so: eleven subcommands map
/// onto five stages, because `markdown`, `html`, `locate`, `overlay` and `tag` report under the
/// phase whose work they project from or write into a file, and `mcp` has no stage at all. Counting subcommands here was wrong twice
