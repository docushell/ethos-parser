...
//! `ethos-parser extract` reads an EPUB, and the page a publisher named stays out (v2-S9).
//!
//! The eighth sibling of `office_cli.rs`, proving the same claim once more: **one subcommand, one
//! artifact type, one serializer**. There is no `ethos.parser.epub.v0` and no `ethos-parser extract-epub`.
//!
...
//! `ethos_parser_office::read`. This file pins that an EPUB gets there, that it is still never told it is
...
fn extract_reads_an_epub_into_the_same_artifact_type_a_pdf_produces() {
    ...
    assert_eq!(
        ...
        "one artifact type — there is no `ethos.parser.epub.v0`"
    );
...
/// **(a) taken at 0.39.0.** `ethos.grounding.v1` 1.1.0 carries the page-less shape, and an EPUB projects into it.
///
/// This passes with **no change to `ethos-parser-grounding`** — which is worth saying for this format
...
#[test]
fn ground_projects_a_page_less_publication_by_name() {
    let dir = tempdir();
    let (_, stdout, _) = extract(&fixture());
