...
//! `ethos-parser mcp` — the first adapter, driven the way a host drives it (v1.2-S1).
//!
...
/// Speak a session to `ethos-parser mcp` over a real pipe and collect one response per line.
///
...
/// The representation `ethos-parser extract` prints, as parsed JSON.
...
fn the_server_completes_a_hosts_opening_handshake() {
    ...
    assert_eq!(init["serverInfo"]["name"], "ethos-parser");
    assert!(
...
///
/// `structuredContent` is the canonical artifact, and it is compared against what `ethos-parser extract`
...
fn the_artifact_through_mcp_is_the_artifact_the_cli_prints() {
    ...
    let round_tripped = ethos_parser_core::c14n_bytes(&result["structuredContent"])
...
fn ground_accepts_the_minted_artifact_and_checks_it() {
    ...
    assert_eq!(
        ...
        "the projection is the one `ethos-parser ground` emits"
    );
