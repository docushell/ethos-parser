...
//! `ethos-parser extract` reads an ODP, and the page it could have had for free stays out (v2-S7).
//!
//! The sixth sibling of `office_cli.rs`, proving the same claim once more: **one subcommand, one
//! artifact type, one serializer**. There is no `ethos.parser.odp.v0` and no `ethos-parser extract-odp`.
//!
...
fn extract_reads_an_odp_into_the_same_artifact_type_a_pdf_produces() {
    ...
    assert_eq!(
        ...
        "one artifact type — there is no `ethos.parser.odp.v0`"
    );
...
///
/// The sharpest case in the ODF family and the reason [`ethos_parser_office::is_odp`] matches exactly
...
///
/// This passes with **no change to `ethos-parser-grounding`** — which matters more for this format than
...
#[test]
fn ground_projects_a_page_less_presentation_by_name() {
    let dir = tempdir();
    let (_, stdout, _) = extract(&fixture());
