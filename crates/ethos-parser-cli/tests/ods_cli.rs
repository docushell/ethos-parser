...
//! `ethos-parser extract` reads an ODS, and the ODF sibling it does not read says so (v2-S6).
//!
//! The fifth sibling of `office_cli.rs`, proving the same claim once more: **one subcommand, one
//! artifact type, one serializer**. There is no `ethos.parser.ods.v0` and no `ethos-parser extract-ods`.
//!
...
fn extract_reads_an_ods_into_the_same_artifact_type_a_pdf_produces() {
    ...
    assert_eq!(
        ...
        "one artifact type — there is no `ethos.parser.ods.v0`"
    );
...
///
/// This passes with **no change to `ethos-parser-grounding`**.
#[test]
