...
//! The oracle harness.
//!
//! `ethos grounding check` is the external oracle for `ethos-parser grounding-check`
...
fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is <repo>/crates/ethos-parser-cli
...
///
/// `ethos-parser-office` joined the workspace at v2-S1 and was never added, so the name said *every*
...
///
/// **Four, not five, and the difference is not an omission.** `ethos-parser-cli` is the crate this
...
fn every_workspace_crate_links() {
    assert_eq!(ethos_parser_core::CRATE_NAME, "ethos-parser-core");
    assert_eq!(ethos_parser_pdf::CRATE_NAME, "ethos-parser-pdf");
    assert_eq!(ethos_parser_office::CRATE_NAME, "ethos-parser-office");
    assert_eq!(ethos_parser_grounding::CRATE_NAME, "ethos-parser-grounding");
    ...
    assert_eq!(
        listed.len(),
        5,
        "Cargo.toml lists {} workspace member(s): {listed:?}. Four of them export a          `CRATE_NAME` this test asserts, and the fifth is `ethos-parser-cli`, which is this test's own          binary crate. A new member needs a line above, or a sentence saying why it has none.",
        listed.len()
    );
    for expected in [
        "ethos-parser-core",
...
fn scratch(name: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "ethos-parser-oracle-{}-{unique}-{name}",
        std::process::id()
    ));
...
fn the_checker_has_no_verification_semantics() {
    ...
    for file in [
        "crates/ethos-parser-grounding/src/check.rs",
        "crates/ethos-parser-cli/src/main.rs",
    ] {
...
fn adversarial_inputs_agree_on_verdict_code_and_path() {
    ...
    let cases: Vec<Break> = vec![
        ...
        (
            ...
            Box::new(|s: &str| s.replacen("\"name\":\"ethos-parser\"", "\"name\":null", 1)),
        ),
        (
