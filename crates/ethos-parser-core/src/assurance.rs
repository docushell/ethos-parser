...
/// format; `ethos-parser-core` never learns what a PDF is (`docs/04-ARCHITECTURE.md` §1).
pub mod codes {
...
impl Capabilities {
    ...
    /// without a code, and `ethos-parser-pdf`'s capability table fails the build when a `true` one
...
mod tests {
    ...
    /// the public c14n rules, was accepted by `ethos-parser ground`, which then emitted a
