    ...
    fn the_default_profile_is_pinned() {
        ...
        assert_eq!(
             ...
             **No representation changes** — this is a projection rule, and `extract` output is \
             ...
             there would be a claim out of a part nobody opened. **That same `-v10` also \
             carries the erasure declaration**, folded into this one move on the owner's \
             decision: a block the document called a heading that comes out as body text \
             is counted in `coverage.structural_erasures` — `heading-level-unresolved-v1` \
             where the element stated no level, `heading-level-unrepresentable-v1` where \
             it stated a depth neither format has. No emitted character changes; the \
             census does, and the census is output. One move rather than two because no \
             release carries either half, so the two are one comparability boundary to \
             every consumer that will ever exist.\n\n\
             **The cost of that fold, recorded rather than hidden.** `origin/main` \
             carried an intermediate `-v10` between `d963edf` and the fold that projected \
             ODF headings without counting the flattened ones, so two commits in this \
             repository\'s own history emit different artifacts under one rule id — the \
             state `crate::html`\'s header calls the one dishonesty a version id exists \
