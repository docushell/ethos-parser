...
//! applies to `ethos-parser verify` exactly as it applies to the other ten subcommands, and
...
/// 1 and 2 apart (`docs/history/03-V0-SCOPE.md` §3.1).
...
pub struct RelayRequest<'a> {
    /// The grounding artifact, as `ethos-parser ground` emits it.
...
/// The adapter id for artifacts `ethos-parser ground` produces.
///
