...
/// test binaries (here and `ethos-parser-cli`, by `#[path]` include), so a crate-relative path would
...
fn schema_path() -> PathBuf {
    repo_root().join("crates/ethos-parser-grounding/schemas/ethos-grounding-source.schema.json")
}
...
fn the_pinned_schema_matches_its_recorded_digest() {
    ...
    let got = ethos_parser_core::sha256_hex_bytes(&bytes);
    assert_eq!(
...
fn the_snapshot_has_not_drifted_from_the_ethos_tree() {
    ...
    assert_eq!(
        ethos_parser_core::sha256_hex_bytes(&ours),
        ethos_parser_core::sha256_hex_bytes(&upstream_bytes),
...
///
/// M6 gave `ethos-parser-grounding` a real `check` module, so "no validation in src/" is no longer the
...
fn the_subset_validator_is_test_only() {
    ...
    for banned in [
        ...
        assert!(
            ...
            "`{banned}` appears in ethos-parser-grounding's library. The production checker mirrors \
    ...
    assert!(
        !repo_root()
            .join("crates/ethos-parser-grounding/src/schema_subset.rs")
            .exists(),
