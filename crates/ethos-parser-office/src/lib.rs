...
//! Revisit only when office or OCR needs a real home — `ethos-parser-office`, `engine-ocr` — and not
//! before."* DOCX is that second format, so this is that home. `ethos-parser-core` never learns what a
//! `.docx` is, exactly as it never learned what a PDF is; the boundary table binds this crate the
//! same way it binds `ethos-parser-pdf`.
//!
//! # The one sentence
//!
...
//! ordinals count per parent — so v2-S3 added nothing to `ethos-parser-core` and is simply the first
...
pub const CRATE_NAME: &str = "ethos-parser-office";
...
fn read_docx(bytes: &[u8], names: &[String]) -> Result<DocumentRepresentation, EngineError> {
    ...
    if !nodes.is_empty() {
        limitations.push(Limitation::document(
            ethos_parser_core::assurance::codes::GEOMETRY_ABSENT_NOT_GROUNDABLE,
    ...
    if embedded > 0 {
        limitations.push(Limitation::document(
            ethos_parser_core::assurance::codes::OFFICE_EMBEDDED_PARTS_NOT_READ,
            format!(
...
/// positionally — see [`ethos_parser_core::OdsLocator`] for why ODF leaves no third option — and the
...
/// [`ethos_parser_core::OdpLocator`] has a field [`ethos_parser_core::PptxLocator`] deliberately does not.
///
...
/// [`ethos_parser_core::NativeLocator::names_a_part`] and [`rtf`].
