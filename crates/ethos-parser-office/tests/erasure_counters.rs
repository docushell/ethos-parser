...
/// Every `.rs` file in `ethos-parser-office/src`, read from the directory.
///
...
fn readers() -> Vec<String> {
    let mut out: Vec<String> = std::fs::read_dir(src_dir())
        .expect("ethos-parser-office/src is readable")
        .flatten()
    ...
    assert!(
        out.len() >= 12,
        "found {} source file(s) in ethos-parser-office/src: {out:?}. Twelve is the number at \
...
/// `ethos-parser-pdf`'s document-level accumulators live in one file, and v2-S9.1 repaired two of them.
///
/// They were guarded by nothing: this file scans `ethos-parser-office/src`, and `extract.rs`'s own test
...
/// wrong wherever a file's first `#[cfg(test)]` is not its last: `crates/ethos-parser-pdf/src/lib.rs`
...
fn the_counter_list_is_complete() {
    ...
    // was a hand-list of eight against `ethos-parser-pdf/src/extract.rs`, derived by nobody, and the
    ...
    assert_eq!(
        ...
        "the document-level accumulators `ethos-parser-pdf/src/extract.rs` declares and the ones \
