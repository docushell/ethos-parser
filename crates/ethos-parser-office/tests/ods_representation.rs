...
fn locators(sealed: &ethos_parser_core::DocumentRepresentation) -> Vec<OdsLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_cell_resolves_to_a_node_addressed_by_the_document_itself() {
    let sealed = ethos_parser_office::read(&fixture("sheet-cells")).expect("the fixture reads");
...
fn the_clean_package_declares_no_erasure_and_the_other_declares_three_kinds() {
    let clean = ethos_parser_office::read(&fixture("sheet-cells")).expect("reads");
    assert!(
        !clean
            .payload()
            .assurance
    ...
    let dirty = ethos_parser_office::read(&fixture("sheet-unread-parts")).expect("reads");
    let detail = dirty
        .payload()
        .assurance
        .limitations
...
fn detection_reads_the_bytes() {
    let bytes = fixture("sheet-cells");
    assert!(ethos_parser_office::is_ods(&bytes));
    ...
    assert!(
        !ethos_parser_office::is_xlsx(&bytes),
    ...
    assert!(!ethos_parser_office::is_ods(&odt));
