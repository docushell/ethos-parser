...
fn locators(sealed: &ethos_parser_core::DocumentRepresentation) -> Vec<OdtLocator> {
    sealed
        .payload()
        .nodes
        .iter()
...
fn a_text_document_declares_no_pages_and_carries_no_page_number() {
    ...
    let content = String::from_utf8(
        ethos_parser_office::zip::read_entry(&bytes, "content.xml")
...
fn a_block_nested_inside_a_kept_block_does_not_reorder_the_artifact() {
    ...
    let sealed = ethos_parser_office::read(&archive).expect("the document reads");
    let addressed: Vec<(u32, u32, &str)> = sealed
        .payload()
        .nodes
        .iter()
...
fn detection_reads_the_bytes() {
    ...
    assert!(ethos_parser_office::is_odt(&odt));
    assert!(!ethos_parser_office::is_docx(&odt));
    assert!(!ethos_parser_office::is_xlsx(&odt));
    assert!(!ethos_parser_office::is_pptx(&odt));
    assert!(ethos_parser_office::read(&odt).is_ok());
    ...
    for (path, kind) in [
        ...
        assert!(
            !ethos_parser_office::is_odt(&other),
    ...
    for not_a_document in [
        ...
        assert!(!ethos_parser_office::is_odt(not_a_document));
    }
}
...
fn the_odf_siblings_are_not_claimed_as_text() {
    for (media_type, kind, claimed_by_ods, claimed_by_odp) in [
        ...
        assert!(
            !ethos_parser_office::is_odt(&archive),
            "an {kind} is a different vocabulary with a different reader"
        );
        assert_eq!(
            ethos_parser_office::is_ods(&archive),
            claimed_by_ods,
...
fn a_mimetype_that_is_not_first_and_stored_is_not_an_odf_package() {
    ...
    assert!(ethos_parser_office::is_odt(&conforming));
    ...
    assert!(!ethos_parser_office::is_odt(&reordered));
    assert!(ethos_parser_office::read(&reordered).is_err());
    ...
    assert!(
        !ethos_parser_office::is_odt(&deflated),
    ...
    assert!(ethos_parser_office::read(&deflated).is_err());
}
...
fn a_conforming_package_with_a_name_ordered_directory_still_reads() {
    ...
    let sealed = ethos_parser_office::read(&archive).expect("a conforming package reads");
...
fn a_package_that_names_one_entry_twice_is_refused() {
    ...
    let error = ethos_parser_office::read(&archive).expect_err("duplicate names are refused");
...
fn a_package_whose_manifest_hides_its_content_is_refused_by_name() {
    ...
    let error = ethos_parser_office::read(&undeclared).expect_err("an undeclared content part");
    assert!(
    ...
    let error = ethos_parser_office::read(&encrypted).expect_err("an encrypted content part");
    assert!(
    ...
    let error = ethos_parser_office::read(&no_manifest).expect_err("a package with no manifest");
    assert!(
        error.to_string().contains("META-INF/manifest.xml"),
        "{error}"
    );
...
fn a_package_that_is_two_formats_is_refused_by_name() {
    ...
    assert!(ethos_parser_office::is_odt(&archive));
...
fn a_zip_that_is_no_office_format_names_every_one_it_is_not() {
    ...
    assert!(!ethos_parser_office::is_odt(&renamed));
