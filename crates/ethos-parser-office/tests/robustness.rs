...
//!
//! Same question as `crates/ethos-parser-pdf/tests/robustness.rs`, asked of packages instead of a byte
...
//! `crates/ethos-parser-cli/tests/oracle.rs` and select fixtures by `owner == "ethos"`, never by root.
...
///
/// **This is not a detection path.** `ethos_parser_office::read` never sees a name — it is handed bytes
/// and decides from what they contain (**A4**), and a test in `ethos-parser-cli` pins that no `src` file
...
///
/// This is the same trap `crates/ethos-parser-pdf/tests/robustness.rs` records for `find_operator`,
...
enum Mutation {
    ...
    /// crate reads** — its only appearance in `crates/ethos-parser-office/src` is inside a `docx.rs`
...
///
/// **Only four are reachable from `ethos-parser-office`** — `malformed`, `unsupported`,
...
///
/// One call rather than the PDF path's four stages: `ethos_parser_office::read` is the whole public
...
fn run_mutant(bytes: &[u8]) -> Result<Read, EngineError> {
    let repr = ethos_parser_office::read(bytes)?;
...
///    reads it** — its only appearance in `crates/ethos-parser-office/src` is inside a `docx.rs` unit
...
///   `ethos_parser_office::read` exists to reconcile, and here it shows up as eight fixtures taking a
...
/// `ethos-parser` entry is `ethos-parser` — which is precisely why someone adding a root could believe the
...
fn adding_this_harness_did_not_touch_the_fixture_manifest_or_the_oracle_count() {
    ...
    assert_eq!(
        ...
        "{office_paths} manifest entry/entries do not end in `.pdf`. Every entry in that file is \
         read by `crates/ethos-parser-pdf/tests/robustness.rs` through `Document::open_bytes`."
    );
}
...
fn a_part_whose_checksum_disagrees_with_its_bytes_is_refused_under_its_own_name() {
    ...
    for fixture in all_fixtures() {
        ...
        let names = ethos_parser_office::zip::entry_names(&bytes).expect("readable directory");
        ...
        let err = ethos_parser_office::zip::read_entry(&damaged, &first)
            .expect_err("a part whose declared CRC does not match its bytes must be refused");

        match &err {
            ethos_parser_core::EngineError::Malformed { what, detail } => {
                assert_eq!(
        ...
        ethos_parser_office::zip::read_entry(&bytes, &first)
...
fn a_length_failure_and_a_checksum_failure_do_not_share_a_name() {
    ...
    let crc_what = match ethos_parser_office::zip::read_entry(&crc_bad, &first) {
        Err(ethos_parser_core::EngineError::Malformed { what, .. }) => what,
    ...
    let len_what = match ethos_parser_office::zip::read_entry(&len_bad, &first) {
        Err(ethos_parser_core::EngineError::Malformed { what, .. }) => what,
        other => panic!("expected a Malformed refusal, got {other:?}"),
    };
