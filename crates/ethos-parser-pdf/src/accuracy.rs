...
pub fn score(
    ...
    for page in &extract.pages {
        ...
        for table in &page.tables {
            s.detected += 1;
            if table.rule == ethos_parser_core::TABLE_DETECTION_UNRULED_V1 {
                s.unruled_detected += 1;
            }
            if declared_pages.contains(&page.index) {
                s.matched += 1;
            }
            if !matches!(table.check.outcome, ethos_parser_core::CheckStatus::Ok) {
...
/// misses. `CellSlot` is `ethos_parser_core`'s own occupancy model, used rather than re-derived.
fn expand<'a>(
    cells: impl Iterator<Item = (u32, u32, u32, u32, &'a str)>,
) -> BTreeMap<ethos_parser_core::CellSlot, String> {
    let mut out = BTreeMap::new();
    for (row, column, rowspan, colspan, text) in cells {
        // `CellSlot` is `ethos_parser_core`'s occupancy type, but the slots are enumerated here rather
        ...
        for r in row..row.saturating_add(rowspan.max(1)) {
            for c in column..column.saturating_add(colspan.max(1)) {
                out.insert(ethos_parser_core::CellSlot::new(r, c), normalize(text));
            }
        }
    }
    out
...
fn geometric_predicted(
    extract: &crate::extract::ExtractArtifact,
) -> Vec<(u32, u32, u32, BTreeMap<ethos_parser_core::CellSlot, String>)> {
    let mut predicted = Vec::new();
    for page in &extract.pages {
        for table in &page.tables {
            predicted.push((
...
fn score_predicted_tables(
    predicted: &[(u32, u32, u32, BTreeMap<ethos_parser_core::CellSlot, String>)],
    labels: &LabelledDocument,
) -> (u32, u32, u32) {
...
mod tests {
    ...
    /// `crates/ethos-parser-pdf/tests/robustness.rs`, so a fourth `benchmark` entry moves the corpus off
    ...
    fn every_gate_document_is_hash_pinned() {
        ...
        assert!(
            ...
            "{} gate document(s) are scored but not hash-pinned: {unpinned:?}\n\
             ...
             `crates/ethos-parser-pdf/tests/robustness.rs`'s pinned totals move with it.",
            unpinned.len()
        );
    ...
    ///
    /// `cargo test -p ethos-parser-pdf --lib -- --ignored regenerate_the_labelled_set`
    ///
    ...
    ///
    ///   `cargo test -p ethos-parser-pdf --lib -- --ignored --nocapture the_ten_documents_where_nothing_is_detected`
    ///
    ...
    fn the_ten_documents_where_nothing_is_detected() {
        ...
        for (root, name) in CORPUS {
            ...
            for d in &diags {
                for t in &d.detected.tables {
                    match t.rule.as_str() {
                        ethos_parser_core::TABLE_DETECTION_UNRULED_V1 => unruled_total += 1,
                        ethos_parser_core::TABLE_DETECTION_STROKE_V1 => stroke_total += 1,
                        _ => ruled_total += 1,
                    }
                }
            }
    ...
    fn tagged_tables_are_emitted_with_typed_absent_geometry_and_no_ok_cross_check() {
        ...
        for t in &tagged {
            assert_eq!(t.rule, ethos_parser_core::TABLE_DETECTION_TAGGED_V1);
