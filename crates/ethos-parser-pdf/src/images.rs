...
use ethos_parser_core::{ImageAttributes, ImageMediaType, PaintedRect, QRect, Sha256Hex};
...
pub fn painted_rect(corners: [(f64, f64); 4]) -> PaintedRect {
    ...
    let q = |v: f64| ethos_parser_core::quantize(v, ethos_parser_core::QUANTUM_PER_POINT).ok();
    let Some(qs) = corners
        .iter()
