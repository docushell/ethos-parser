...
const EXPECTED_SURVIVORS: [&str; 74] = [
...
fn every_fixture_is_mutated_and_the_coverage_is_reported() {
    ...
    assert_eq!(
        fixtures.len(),
        78,
        "the manifest should declare 78 fixtures across FOUR roots. S1's review moved this from \
         76 by adding the `/OBJR` pair — `tagged-widget-objr` and `engine-tagged-widget-objr` — \
         the form-field-value page under a structure tree whose one content item cites the \
         widget by object, so the locator the `/OBJR` arm mints is read by a test in both \
         classes. Auto-tagging S1 moved this \
