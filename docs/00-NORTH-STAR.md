...
| **v2.2** | **Closed at half by row 21**: a region is emitted wherever the cut divided a page and nowhere else — **met**; a tag this engine writes is one it can read back and ground against — **refused** | Layout and accessibility — geometric block structure shipped, auto-tagging refused on the format |
...
never gets one**, by row 21: it is refused rather than unstarted, and a scope document for refused
work is a plan for something nobody may build. v3 and v4 are still one line each, deliberately.
