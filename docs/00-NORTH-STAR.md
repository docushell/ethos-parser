...
**Row 18 is the exception and is NOT settled.** It is written here because v2-S19 gathered the
evidence the question was waiting on, and a decision with its evidence attached is easier to make
than one without. It is the owner's, exactly as #16 and #17 were. Until it is taken, nothing in
this repository may cite it as decided, and v1 stays open.
...
| 16 | **The v2 gate's verb, settled by the owner 2026-08-21: *ground* means *bind*.** The gate reads "a DOCX quote and an XLSX cell both **bind**" — each resolves to an address the file itself states, which is what the eight readers emit and have since v2-S3. It does **not** mean emitting `ethos.grounding.v1`, which a DOCX cannot: `source.media_type` is a `const` of `application/pdf`, every element requires a `page`, and every page requires integer geometry. The decisive argument is that the gate's own second half forbids what the literal reading requires — grounding needs pages, and *"no synthesised pages"* is the same sentence. Read literally the gate contradicts itself; read as *bind* it is met. Option (a), widening the verifier's schema, stayed **blocked on an Ethos-side revision owned elsewhere** rather than refused — until that revision landed (`ethos.grounding.v1` 1.1.0, made on the Ethos side first) and 0.39.0 took it: office representations project the page-less shape, with no synthesised page anywhere. The verb is unchanged — *ground* still means *bind*, and the wire caught up to the verb. |
...
| 18 | **NOT SETTLED — for the owner. What v1's table number is, now that twelve documents have measured it — and now that the engine reads tables two ways.** v1-S7 has been *"not complete"* since v1-S7b, and the reason given was a corpus nobody was acquiring. v2-S19 acquired it: **twelve tagged, public, redistributable documents — 2 068 pages, 172 tagged tables, 15 755 tagged cell slots** — under an admission rule written into `docs/table-gate-v1.md`. **The evidence.** Macro cell-slot F1 moved **64‰ → 70‰** across a threefold corpus, which read alone is stability. The band says otherwise and the band is what matters: **0‰ .. 590‰, median 0‰, and ten of the twelve score exactly 0‰** — not "low" but zero, because the detector emits no table at all on them. Two documents supply **all 849** of the averaged points, and **removing `irs-fw9` alone drops the macro to 23‰**, a threefold move from one document out of twelve. **Micro recall says it without needing a band: 4‰**, 70 of 15 755 gold slots (v2-S22). So the four-document 64‰ was never a property of this engine and the twelve-document 70‰ is not one either; what twelve documents establish that four could not is the **shape**, which is bimodal — the geometric rules work where a producer drew the grid and produce nothing where they did not. **Fabrication is 0 on all twelve**, the one number with a required value, and it holds. **Two paragraphs of this evidence have since been overtaken. Both are recorded rather than deleted, because what changed them is the answer to what this entry asked for.** *(1) The phantom grids are gone.* This entry read *"new and worse than a zero: `nist-sp-800-218` detects nine phantom grids up to 103 × 22 against four tagged tables, contributing 11 295 false-positive slots — and the engine's own locator cross-check already flags all nine"*, and offered that as Option B's concrete lead. **v2-S20 took exactly that lead**, making the ruled rule decline a grid its own structural cross-check rejects: that document now emits nothing, detection precision reads **941‰**, and cross-check disagreements are back to **0**. The macro did not move. *(2) The gap the band describes is now closed from the other side.* **v2-S24** added `tagged-tables-v1`, which emits the tables the documents **declare** in their structure trees rather than the grids they **draw** — **157 of the 172 gold tables**, **15 593 cells with 0 fabricated** — moving combined micro recall **4‰ → 502‰**. The geometric gate deliberately stays at 70‰/4‰: scoring a tree-derived table against the tree it came from is circular, so the two are measured apart. **Option A — accept a number and close v1.** Defensible, but only if what is published is the band and the capability rather than the macro: a single averaged figure over mostly zeros will be read as *"the engine gets 7% of table cells right"* when the geometric truth is *"it gets most of two documents right and nothing on ten"*. **Option B — un-park the v1-S7 chase.** The concrete lead this entry offered has been **spent**: v2-S20 took it, and it improved precision without moving the macro. Re-opening now means a **new** geometric idea, not this one. **The question v2-S24 puts to both options, and the one that has to be answered first:** whether *"v1's table number"* means the **geometric detectors alone** — what the gate measures, reading 70‰ macro and 4‰ micro — or the **engine's table output**, which now includes what the documents declare and reads **502‰**. That is a definitional choice about what v1 ever claimed, not a measurement, which is why no slice has taken it. **Recommendation, unchanged in shape and updated in content:** close v1 on an honest **capability statement plus the band** rather than on a macro — now two clauses rather than one, because the engine now does two things: *"reads the tables a document declares in its structure tree; detects ruled tables where the producer drew the rules; emits nothing where neither holds; fabricates nothing."* Closing v1 needs an honest number rather than a good one, and the honest one is now available in both readings. |
...
| **v1** | Fabrication rate **0** and an honest table number on the set it owns — the geometric detectors read **70‰ macro / 4‰ micro over twelve tagged PDFs** (v2-S19), band 0‰..590‰, median 0‰, **ten of twelve at zero**. v2-S24 added `tagged-tables-v1`, which reads the tables the documents *declare* rather than the grids they *draw*: **combined micro recall 502‰**, fabrication still 0, and the geometric gate deliberately unmoved (scoring a tree-derived table against the tree is circular). The **> 0.489** chase is **parked** (decision #10). **Not complete** — #18, which would close it on that evidence, is written and **undecided** | The DocuShell replacement gate — tables, full element vocabulary, tagged PDF, forms, vectors |
...
What 0.35.0 can and cannot do, on one page: `CAPABILITY.md`. The research archive that produced
