...
| 18 | **NOT SETTLED — for the owner. What v1's table number is, now that twelve documents have measured it.** v1-S7 has been *"not complete"* since v1-S7b, and the reason given was a corpus nobody was acquiring. v2-S19 acquired it: **twelve tagged, public, redistributable documents — 2 068 pages, 172 tagged tables, 15 755 tagged cell slots** — under an admission rule written into `docs/table-gate-v1.md`. **The evidence.** Macro cell-slot F1 moved **64‰ → 70‰** across a threefold corpus, which read alone is stability. The band says otherwise and the band is what matters: **0‰ .. 590‰, median 0‰, and nine of the twelve score exactly 0‰** — not "low" but zero, because the detector emits no table at all on them. Two documents supply 849 of the 851 averaged points, and **removing `irs-fw9` alone drops the macro to 23‰**, a threefold move from one document out of twelve. So the four-document 64‰ was never a property of this engine and the twelve-document 70‰ is not one either; what twelve documents establish that four could not is the **shape**, which is bimodal — the ruled rule works where a producer drew rules and produces nothing where it did not. **Fabrication is 0 on all twelve**, the one number with a required value, and it holds. **New and worse than a zero:** `nist-sp-800-218` detects nine phantom grids up to 103 × 22 against four tagged tables, contributing 11 295 false-positive slots — real text arranged into a grid that is not there, not invented text — and **the engine's own locator cross-check already flags all nine** (disagreements went 0 → 9 on this corpus). **Option A — accept a number and close v1.** Defensible, but only if what is published is the band and the capability, not the macro: a single averaged figure over mostly zeros will be read as "the engine gets 7% of table cells right" when the truth is "it gets most of two documents right and nothing on nine". **Option B — un-park the v1-S7 chase.** Now backed by evidence rather than preference, and with a concrete lead this corpus produced: the cross-check already rejects the phantom grids, so a rule that declined a table its own cross-check rejected costs no new geometry. That is a **detector change**, which S19 was forbidden to make. **Recommendation:** close v1 on an honest **capability statement plus the band** — *"detects ruled tables where the producer drew the rules; emits nothing otherwise; fabricates nothing"* — rather than on the macro, and treat the `nist-sp-800-218` cross-check signal as the first lead if the chase re-opens. Closing v1 needs an honest number, not a good one, and the honest one is now available. |
