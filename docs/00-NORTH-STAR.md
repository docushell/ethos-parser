...
**Row 18 is the exception — it is open, and it is the owner's.** Until it is decided, nothing in
this repository may cite it as settled, and v1 stays open.
...
| 18 | **OPEN — for the owner. What is v1's table number, now that twelve documents have measured it and the engine reads tables two ways?**<br><br>**The evidence.** The corpus is twelve tagged, public, redistributable documents: 2,068 pages, 172 tagged tables, 15,755 tagged cell slots. The geometric detectors score 70‰ macro cell-slot F1 — but the band is 0‰..590‰ with a median of 0‰, and **ten of the twelve score exactly zero**, because the detector emits no table at all on them. Two documents supply the entire average, and removing one drops the macro to 23‰. Micro recall says it without needing a band: **4‰**, 70 of 15,755 slots. **Fabrication is 0 on all twelve.**<br><br>So neither 64‰ nor 70‰ was ever a property of this engine. What twelve documents establish that four could not is the **shape**, and it is bimodal: the geometric rules work where a producer drew the grid, and produce nothing where they did not.<br><br>**Since this row was written, two things changed it.** v2-S20 made the ruled rule decline any grid its own cross-check rejects, which removed nine phantom grids from one document and took detection precision to 941‰ with cross-check disagreements back to 0 — without moving the macro. And v2-S24 added `tagged-tables-v1`, which emits the tables documents **declare** in their structure trees rather than the grids they **draw**: 157 of the 172 gold tables, 15,593 cells, 0 fabricated, taking combined micro recall from 4‰ to **502‰**. The geometric gate deliberately stays where it is, because scoring a tree-derived table against the tree it came from is circular.<br><br>**The question that has to be answered first** is definitional, not empirical: does *"v1's table number"* mean the **geometric detectors alone** (70‰ macro, 4‰ micro) or the **engine's table output**, which now includes what documents declare (502‰)? No slice has taken that because it is a choice about what v1 ever claimed.<br><br>**Option A — accept a number and close v1.** Defensible, but only if what gets published is the band and the capability rather than the macro. A single average over mostly zeros reads as *"the engine gets 7% of table cells right"* when the truth is *"most of two documents, nothing on ten"*.<br><br>**Option B — un-park the chase.** The concrete lead this row once offered has been spent: v2-S20 took it, and it improved precision without moving the macro. Reopening now means a genuinely new geometric idea.<br><br>**Recommendation:** close v1 on an honest capability statement plus the band, not on a macro — now in two clauses, because the engine does two things: *reads the tables a document declares in its structure tree; detects ruled tables where the producer drew the rules; emits nothing where neither holds; fabricates nothing.* |
...
| **v1** | Fabrication is 0 and there is an honest table number on the set this repo owns. **Not complete** — decision #18 is written and undecided | The DocuShell replacement gate |
