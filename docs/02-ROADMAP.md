...
four). **Quote the band with it:** 0‰ .. 590‰, median 0‰, **ten of twelve score exactly 0‰**, and
...
| What is v1, and did its gate clear? | `08-V1-SCOPE.md` / `09-V1-MILESTONES.md` — **measured at 70‰ over twelve documents, a miss — and the band (0‰..590‰, ten of twelve at zero) matters more than the macro; the 0.489 chase is parked, and #18 is written and undecided** |
...
| What is v2 (office formats)? | `14-V2-SCOPE.md` / `15-V2-MILESTONES.md` — **S0 through S24 done, at 0.37.0**: the mutation lane at 0.32.0, guards at 0.32.1, the roadmap reorder at 0.32.2, prose at 0.32.3, the owner's two gate decisions at 0.32.4, the two unrun sweeps at 0.32.5, the CRC-32 answer at 0.33.0, the guards those sweeps named at 0.33.1, the `neither detector` cluster at 0.34.0, the no-behaviour-change extractor itself at 0.34.1 the two guards outside `src` at 0.34.2 the gate that has never been green at 0.34.3 the corpus that was never grown at 0.35.0 — which re-measured the table gate on twelve documents instead of four and escalated decision #18 to the owner, undecided — and the nine grids the engine already rejects at 0.36.0, the first detector change since v1-S8: a grid whose own structural cross-check rejects it is refused rather than emitted, which removes 11 295 false-positive cell slots and moves the macro not at all, and the mutation that missed at 0.36.1 — `flip-tail-byte` now seeks the file trailer's cross-reference pointer instead of a fixed fraction of the file's length, which it had claimed to reach since M7 and landed 373 KB short of on the largest fixture, so all eighteen of its survivors now fail closed, and why ten documents produce nothing at 0.36.2 — a diagnostic that measures, per gold table, why ten of the twelve gate documents detect no table at all, finding that `unruled-align-v1` has emitted none on any real document, so v1's remaining gap is the ruled and stroke-ruled rules alone; it changes no detector and states micro recall (4‰) beside the macro (70‰), and the coverage two slices retired at 0.36.3 — v2-S23 verifies `CheckStatus::Mismatch` is still reachable on the wire through a geometric-only `CellsOverlap` fault (pinned in `near_edges_fold_into_one_lattice_line`) and argues `lopdf`'s catalog-scan recovery for deletion rather than a fixture, since it is a backend leniency the engine never guaranteed; it changes no reader, `ci/code-lines.py` diffs empty, and the tagged tables the documents declare at 0.37.0 — v2-S24 adds a **fourth** detection rule, `tagged-tables-v1`, emitting a table for each `/Table` the structure tree declares that no geometric detector matched: `Extracted` rather than `Computed` because the document stated the grid, geometry typed-absent with no box invented, its cross-check not-applicable rather than `ok`, fabrication still 0 across 15 593 tagged cells, and combined micro recall moved 4‰ → 502‰ while the geometric gate stays at 70‰/4‰ because scoring a tree-derived table against the tree it came from is circular. S10 is CSV as an argued refusal, not a reader. **v2's format row is closed and the gate is MET** (decisions #16, #17); the last question — `zip.rs`'s CRC-32 — was answered at **S14, as 0.33.0**, measured before it shipped |
