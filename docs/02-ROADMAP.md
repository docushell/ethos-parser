...
One page, deliberately. Ten versions, one line each. No new version number gets invented without a
decision recorded first in [`00-NORTH-STAR.md`](00-NORTH-STAR.md) — every change from research folds
into a row that already exists. **v2.3 is the one invented, by decision #28 (2026-09-17)**; until
then this rule allowed no exception.
...
[`24-AUTO-TAGGING-MILESTONES.md`](24-AUTO-TAGGING-MILESTONES.md)). **v2.3** (decision #28) has
neither yet; whether its independent items get them as one version or one by one is the owner's
([`OPEN-WORK.md`](OPEN-WORK.md) §4). **v3 and v4 stay one line each**, which is what "one page"
protects.
...
| **v2.2** | Layout and accessibility | **Geometric block structure** — the reading-order cut's own column bands, emitted per node as `Computed` layout · **the block cut, shipped 0.55.0** as `TextRunAttributes.block` under `gutter-columns-v3` — the unnamed `Computed` index [`19-BLOCK-SUBDIVISION-SCOPE.md`](19-BLOCK-SUBDIVISION-SCOPE.md) §6 permits, and shipped **without the scope document this page requires before code**; its limits ride on every PDF artifact as `block-subdivision-leading-gap-only` · **auto-tagging, shipping in 0.59.0**: the reader binds an engine-written `/Div` as `Computed` under `struct-tree-v2`, and the writer, the `tag` subcommand, fills absence only — scope [`23-AUTO-TAGGING-SCOPE.md`](23-AUTO-TAGGING-SCOPE.md), slices [`24-AUTO-TAGGING-MILESTONES.md`](24-AUTO-TAGGING-MILESTONES.md) | **Met**, on decision #27 (2026-09-17). Clause one — a region is emitted wherever the cut divided a page and nowhere else — **met** at 0.42.0. Clause two — a tag this engine writes is one it can read back and ground against — was closed by #21, because a `/P` written from a `Computed` cut reads back as `Extracted` and is uncorrectable, and reopened by #23 with an attribute object under `/A` carrying the derivation. **It is met on the fixtures**: the reader binds an engine-written `/Div` as `Computed` (`struct-tree-v2`), the writer `tag` fills absence only, and the block cut is its consumer; S3's round-trip tests pass, and S4 published the round trip over 293 documents — 129 tagged, each read back as computed, projected and grounded as its untagged original — in [`measurements/auto-tagging/`](measurements/auto-tagging/) §2. Decision #25 records the shape and #26 keeps the writer off MCP and the SDKs. It ships in 0.59.0 |
| **v2.3** | Distribution | Cross-OS byte identity, compared in CI on Linux, macOS and Windows · GitHub Releases for three platforms, each binary run before it is labelled `verified` · the memory ceiling · **`locate(representation, quote)`**, on the engine's side of the verify boundary (decision #30), scoped in [`26-LOCATE-SCOPE.md`](26-LOCATE-SCOPE.md) and **built**: the core query and `ethos.parser.locations.v0` (S1, with the cap measured at 127.6 MiB — [`measurements/locate/README.md`](measurements/locate/README.md)), then the eleventh subcommand, a fourth MCP tool and `locate()` in both SDKs (S2), slices in [`27-LOCATE-MILESTONES.md`](27-LOCATE-MILESTONES.md) · the knobs, settled (decision #31) · PyPI, npm and crates.io packages, the owner's to publish, with D8 deferred | A user installs a prebuilt binary on any of three platforms and gets a byte-identical artifact on all three. **Added by decision #28** (2026-09-17), which dropped the local plan's second condition, *a quote grounded to its own glyphs*, on [`22-WORD-BOXES-SCOPE.md`](22-WORD-BOXES-SCOPE.md)'s refusal of word boxes. **Met on measurement 2026-09-20, and the distribution step remains.** The `v0.59.0` and `v0.60.0` tag pushes each built, executed and verified four targets — Linux, Windows and both macOS architectures — with one fingerprint across all four runners, which is this row's condition measured on the binaries a user would install. What remains is the install itself: the owner attaches each bundle to its release. Before those runs: *Both workflows exist and have never run* stood until 2026-09-18, when `main` was first pushed with them: **`cross-os byte identity` ran and passed** (run 35318173062, on `791f0fe`) — CI builds on Linux, macOS and Windows produce byte-identical artifacts over the fixture corpus, the operating-system axis of this row's condition. The other half — prebuilt release binaries, each executed where it was built and verified identical across runners — waits for **0.59.0, the first release `release-artifacts.yml` can build**: the same push showed that file was invalid (fixed that day), and the 0.58.0 commit predates it (`RELEASING.md` §8) |
...
| **Auto-tagging — writing Tagged PDF (v2.2's second half)** | **No longer refused — reopened by [decision #23](00-NORTH-STAR.md) on 2026-09-07, and built ([`23-AUTO-TAGGING-SCOPE.md`](23-AUTO-TAGGING-SCOPE.md)); its shape is recorded as decisions #25–#27 (2026-09-17).** [Decision #21](00-NORTH-STAR.md) had refused it on the format: a `/P` written from a `Computed` cut reads back as `Extracted` and `may_be_overwritten_by` makes it **uncorrectable**. #23 answers the reopening condition with an attribute object under `/A` naming a private owner — in the file rather than in a doc comment, and a shape `structure.rs` parses today. The guarantee is **engine-local**: another reader still sees a plain structure element — #23 wrote `/P`, and the writer as built emits `/Div` ([`23-AUTO-TAGGING-SCOPE.md`](23-AUTO-TAGGING-SCOPE.md) §3.2) — and the format still has no standard field |
...
| **A role, heading or paragraph read off a region** | A region is *where*, never *what*. P14 — and the whole reason layout and structure are separate axes in decision #19. Decision #29 admits a heading inferred from font size or font name where a document declares no structure; it reads type, never a region, and P14 stands for every other role |
...
| What is v2.2's second half? | [`23-AUTO-TAGGING-SCOPE.md`](23-AUTO-TAGGING-SCOPE.md) / [`24-AUTO-TAGGING-MILESTONES.md`](24-AUTO-TAGGING-MILESTONES.md) — auto-tagging: one `/Div` per block of the cut, marked computed under `/A` and read back as `Computed`; S1 to S4 done, for 0.59.0 — the reader, the writer and `tag`, the round trip, and its measurements in [`measurements/auto-tagging/`](measurements/auto-tagging/); the shape is recorded as decisions #25–#27 |
| What are inferred headings, exactly? | [`28-HEADINGS-SCOPE.md`](28-HEADINGS-SCOPE.md) — decision #29's rule: where a document declares no structure, a line whose type is larger than the document's own body type is a heading, `Computed`, with the rendered em as the signal, a 5% per-document false-positive bound set before the code, and P14 standing for every other role. No version carries it yet ([`OPEN-WORK.md`](OPEN-WORK.md) §4) |
| What is `locate`, exactly? | [`26-LOCATE-SCOPE.md`](26-LOCATE-SCOPE.md) / [`27-LOCATE-MILESTONES.md`](27-LOCATE-MILESTONES.md) — decision #30's shape: where a string lies in a representation, `ethos.parser.locations.v0`, the match rule `locate-scalar-exact-v1`, and every verdict-shaped convenience refused by name. S0 done; the code is S1 and S2 |
| What is v2.3? | Decision #28 in [`00-NORTH-STAR.md`](00-NORTH-STAR.md) — distribution. No scope or milestones document for the version yet (pending, [`OPEN-WORK.md`](OPEN-WORK.md) §4); [`25-KNOBS-SCOPE.md`](25-KNOBS-SCOPE.md) scopes the knobs; `ci.yml` and `release-artifacts.yml` carry cross-OS identity and the release binaries; §3 of the ledger tracks every item |
...
| How does this engine score on a corpus it does not own? | [`measurements/opendataloader-bench/`](measurements/opendataloader-bench/) — an instrument, never a ranking (O26). NID 0.8697, TEDS 0.1704, MHS 0.0000 at 0.58.0, each with its band and the caveat that makes it readable |
| Can the cut find blocks, not just columns? | [`19-BLOCK-SUBDIVISION-SCOPE.md`](19-BLOCK-SUBDIVISION-SCOPE.md) — measured, three probes run, **never scoped, and shipped anyway**: the cut shipped at 0.55.0 as `TextRunAttributes.block` under `gutter-columns-v3` (`blocks.rs`), without the scope document the paragraph below requires before code. §6 there is where its refusal of roles stands — an unnamed `Computed` index, never a paragraph — and its limits are declared on every PDF artifact as `block-subdivision-leading-gap-only`. Decision #21 removed the consumer it was for and #23 gave it back; the document's amendments record the ship |
...
| What is still open, and what does each item wait on? | [`OPEN-WORK.md`](OPEN-WORK.md) — the ledger: v2.3's items (decision #28), owner decisions, recorded defects |
...
second half was scoped exists now. **v2.3's** documents are pending with the owner
([`OPEN-WORK.md`](OPEN-WORK.md) §4). **v3 and v4 still should not get one** — a scope document for
