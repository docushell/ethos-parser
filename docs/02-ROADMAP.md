...
[`24-AUTO-TAGGING-MILESTONES.md`](24-AUTO-TAGGING-MILESTONES.md)). **v2.3 has neither as a whole**,
by decision #28: its items are independent, and each item that changes the engine gets its own
scope. **v3 and v4 stay one line each**, which is what "one page" protects.
...
| What is v2.3? | Decision #28 in [`00-NORTH-STAR.md`](00-NORTH-STAR.md) — distribution, with no scope or milestones document for the version as a whole. [`25-KNOBS-SCOPE.md`](25-KNOBS-SCOPE.md) scopes the knobs; `ci.yml` and `release-artifacts.yml` carry cross-OS identity and the release binaries; `locate` and inferred headings each get a scope before their code; [`OPEN-WORK.md`](OPEN-WORK.md) §3 tracks every item |
...
second half was scoped exists now. **v2.3 is the second exception, taken by decision #28 rather
than found afterwards**: a version of independent items gets no scope or milestones document as a
whole, and each item that changes the engine gets its own scope before its code. **v3 and v4 still
should not get one** — a scope document for work in progress is a plan, and one for work not yet
started is a guess.
