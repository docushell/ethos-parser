...
| | **Amended 2026-09-20: two such codes shipped, and this refusal is untouched by them.** `heading-level-unresolved-v1` and `heading-level-unrepresentable-v1` count a heading the **document declared** that the projection could not carry — structure that arrived on the wire and did not reach the output, which is what the census is for and the exact opposite direction of the row above. An inferred heading still gets no code here, for the reason stated: it is structure the projection added, and the census that names it is `heading_inference_rule` on the profile plus the document-scoped declaration (§5.1) | |
...
| | **Amended 2026-09-20 (second amendment, later the same day): the flattening that remains is now counted.** A `<text:h>` that states no level still projects as a paragraph — that is the row below, unchanged — but the artifact no longer passes over it in silence: `heading-level-unresolved-v1` carries one count per such block, in both projections, under `markdown-blocks-v10` / `html-blocks-v10`, the same id as the projection itself, folded on the owner's decision. `heading-level-unrepresentable-v1` is its pair, for a stated depth past the six either format can write. Neither carries a `gfm-` prefix, because GFM is not what causes them | |
| | **Amended 2026-09-20: the gap is closed and the row above is the record of what was true before it.** `text:outline-level` is read, carried on both ODF attribute structs and projected — `markdown-blocks-v10` / `html-blocks-v10`. Nothing in the *inference* column changes: the gate still closes on ODF because the format declares its own structure, and no type size reaches these readers. What changed is that the declaration now arrives whole rather than halfway. A `<text:h>` that states no level still projects as a paragraph, which is the §11 row below, answered | |
...
| **Amended 2026-09-20, second: what the slice left behind is declared rather than silent.** A `<text:h>` with no stated level still projects as a paragraph, and that outcome is now counted — `heading-level-unresolved-v1`, one per block, on both artifacts | Standing rule #3, *"No silent drop and no silent repair"*. The `#` that was never written is not a dropped character, so the character census cannot carry it and the second census must | A slice that opens `styles.xml`, which would drive this count toward zero on the same documents — the code's own retirement condition |
| **Amended 2026-09-20: it reopened, and it was that slice.** Read, carried, match arm added — and the question this row said had to be settled was settled against defaulting: an absent `text:outline-level` is **not** level 1, because the level a bare `<text:h>` displays at is resolved through `styles.xml`, which the reader declares unread on the same artifact | The prediction held exactly: it was a reader slice, and the settling was the hard part rather than the reading | A corpus of real ODF packages showing that producers omit the attribute often enough for the refusal to cost real headings — which no measurement in this repository can answer today, because every corpus under `docs/measurements/` is PDF |
