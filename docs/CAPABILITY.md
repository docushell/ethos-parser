...
**Version in this file, not in its name:** these two tables describe **0.27.0**. When the workspace
...
| **Extract (office)** | **DOCX, XLSX, PPTX, ODT, ODS, ODP, RTF** into the same representation, with **structural** locators, every field a thing the file contains — `{part, paragraph, run}`, `{part, sheet, row, column}`, `{part, shape, paragraph, run}`, `{part, paragraph}`, `{part, table, row, column}`, `{part, draw_page, shape, paragraph}`, `{paragraph}`. Each is `deny_unknown_fields`. The ODS row and column are **positions**, because OpenDocument writes no address at all; so is the ODP draw page, which is named for the `<draw:page>` element rather than for the page it resembles. **RTF names no part at all**, because it has none — see `15-V2-MILESTONES.md` S6, S7 and S8 | `engine extract` |
| **Office pages** | **`pages: []`, always.** No office format synthesises a page — not a spreadsheet's print range, not a slide, not an ODT's or an ODS's `<text:soft-page-break/>`, not a `<draw:page>` with a master page's `fo:page-width` beside it, which is the one case where a `PageRecord` needed no arithmetic at all, and **not RTF's `\page`, which says the word outright** (row **L30**) | — |
...
| **"v2 is complete"** | not yet | S0–S8 are done. **S9 (EPUB, CSV) has not started** |
...
| **EPUB, CSV** | not yet — **S9** | An `.epub` shares OCF's first-and-stored `mimetype` rule and is deliberately **not** told it is OpenDocument (v2-S6's pin). Since v2-S8 any unread ZIP is refused naming the **container** rather than a missing `%PDF-` header. A `.csv` still takes the unknown-bytes path, because comma-separated text cannot be told from prose without a reader and a detector that guessed would claim every comma file |
