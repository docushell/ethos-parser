...
**Version in this file, not in its name:** these two tables describe **0.29.1**. When the workspace
...
| **"v2 is complete"** | not yet | S0–S10 are done and **v2's format row is closed** — eight formats read, one argued refusal, every split against a measurement. Two things remain, neither a format and both the **owner's** (`15-V2-MILESTONES.md` S10): the gate sentence's verb, which says a DOCX quote **grounds** while grounding a DOCX is refused and every slice since S1 has read it as **binds**; and **embedded assets**, a roadmap obligation neither delivered nor descoped, and not merely unread but **uncounted** — `word/media/` matches no A14 bucket, so a DOCX with forty images declares zero unread parts for them |
