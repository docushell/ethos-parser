...
| **Cut 0.61.0** | **release commit on local `main`** | A MINOR, because readers changed: the PDF catalog's `/Outlines` is read under `outlines-v1` onto its own record — never as `Node`s, so an outline title is deliberately not quotable — and an ODF `<text:h>` projects at the `text:outline-level` it declared under `markdown-blocks-v10`/`html-blocks-v10`. Three further changes move no byte of any artifact and put on the wire what the tree already knew: `text_box_rule: advance-over-font-envelope-v1` (contract §5.3, closed), and the `right-to-left-not-reordered` and `document-metadata-not-read` limitations. `REPRESENTATION_SCHEMA_VERSION` 0.6.0 → 0.7.0, `EXTRACT_SCHEMA_VERSION` 0.4.0 → 0.5.0. `profile_sha256` re-pinned at 0.61.0 (`sha256:78fcfe87e9…`), which is its fourth move this cycle and the only one that is the version alone; the three worked examples regenerated together, and they were still carrying `sha256:850e4fa3…` from before the three feature moves. Remaining, the owner's: create the GitHub Release from the bundle the tag push builds |
