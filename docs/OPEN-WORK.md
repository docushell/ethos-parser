...
| Decision | Blocks |
| --- | --- |
| **Does the heading rule ship as measured?** C1 S3 ran the shipped `type-size-v1` over the eleven documents whose authors declare headings ([`measurements/headings/README.md`](measurements/headings/README.md)): the bound is met by its letter — **0.00%..4.61%** over the nine, worst `cfpb-home-loan-toolkit` — and on the two forms the rule is excellent (`irs-fw9` 26 of 28 headings, no false positive). But two documents carry **2,979 false headings against 68 declared**: `nist-sp-800-218` at 28.84% (counts-only by §7.5, which reserves its breach for the owner) and `nist-sp-800-53Ar5` at 3.05% of 80,090 lines, both because the char-weighted body em is dense small type, so ordinary prose clears the cut. Options: ship as measured; repair the reference (the largest common size, not the most common) or add a common-size refusal, each a `-v2` re-measured here; or bound the fired count against the declared count as well as the rate. S1 `b06b2fc` and S2 `bf44152` are local and unpushed | Pushing C1; S4's MHS band is measurable either way |
...
- **Headings inferred from font size or font name** (decision #29) — **scoped 2026-09-18**,
  ...
  hash, and it fires on none of the 58 pre-existing engine fixtures that extract. **S2, the projections, done the same day**: an inferred heading is `#` and `<h1>` under `-v8`, the gate documents' projections are unchanged but for the ids and digests, and an engine-tagged page keeps its heading. **S3 measured the same day**: the bound is met by its letter and breached in purpose on two documents — 2,979 false headings against 68 declared — which §7.5 leaves to the owner (§4 above). S4 (MHS) next; S5 is conditional. What the scope settles, each on
