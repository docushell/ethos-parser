...
| **Read the document's own `/Outlines` (PI-A).** Reopen [`17-D1-SCOPE.md`](17-D1-SCOPE.md) §5's evidence bar on [`29-OUTLINES-SCOPE.md`](29-OUTLINES-SCOPE.md) §2.1's measurement. §3 there refused `/Outlines` as a *document-boundary* signal and §6 conditioned the refusal on a boundary-shaped **name**; an outline emitted as an outline is neither, and P14 does not bear on it because a `/First`/`/Next` chain is the author's declaration, not presentation. **Measured 2026-09-23:** 6 of 70 fixtures carry one, holding **2 273 entries**, max declared depth 5, **0 unresolvable destinations**, 2 backward-stepping, 0 empty titles, **0 truncated walks** — the count confirmed twice by independent routes. D1's own band was a single point at zero on 45 fixtures. **Not free:** a ninth `RepresentationPayload` field bumps `REPRESENTATION_SCHEMA_VERSION` 0.6.0 → 0.7.0 and moves every artifact's bytes (§5), and 69 of the 2 273 titles carry bytes this engine cannot decode without vendoring Annex D.2's 0x80–0x9F block, which `21-STANDARD-14-ASCII-COVERAGE-SCOPE.md` §4 refused once — so the scope leaves those titles **absent and counted** rather than putting that refusal on the critical path | The outline record, and `29-OUTLINES-SCOPE.md` S1–S3 |
