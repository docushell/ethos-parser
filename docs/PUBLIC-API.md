...
| Detection | `is_docx`, `is_xlsx`, `is_pptx`, `is_odt`, `is_ods` — **content, never extension** (Anydoc's **A4**) · `is_opendocument` — the ODF **family**, implemented or not |
| Media types | `DOCX_MEDIA_TYPE`, `XLSX_MEDIA_TYPE`, `PPTX_MEDIA_TYPE`, `ODT_MEDIA_TYPE`, `ODS_MEDIA_TYPE` |
| Readers | `docx` (runs of `word/document.xml`), `xlsx` (sheets, shared strings, cells), `pptx` (slides, shapes, runs), `odt` (the manifest, and `content.xml`'s paragraphs), `ods` (`content.xml`'s tables, rows and cells) |
...
XML-reader plumbing all **five** readers share — the two ODF readers included, which between them
reach eight of its items —
