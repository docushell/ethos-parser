...
**Status:** frozen at M7 (v0.1.0) · **Enforced by:** `crates/ethos-parser-cli/tests/public_api.rs`
...
The freeze is **narrowing only**. M7 removed items from the surface; it added `ethos-parser-core`'s
...
1. **`ethos-parser-cli` exports nothing.** It is a binary. There is no `ethos_parser_cli::` anything, and the
...
| `html` | `to_html`, `HtmlArtifact`, `HTML_ARTIFACT_TYPE`, `HTML_SCHEMA_VERSION`, `HTML_RULE_BLOCKS_V10` |
| `markdown` | `to_markdown`, `AnchorMap`, `Segment`, `SegmentKind`, `Coverage`, `DroppedBucket`, `StructuralErasure`, `MarkdownArtifact`, `MARKDOWN_ARTIFACT_TYPE`, `MARKDOWN_SCHEMA_VERSION`, `MARKDOWN_RULE_BLOCKS_V10`, `GFM_SPAN_SLOTS_UNREPRESENTABLE`, `GFM_ROW_ZERO_SEPARATOR`, `GFM_CELL_RUN_CLAIMED_TWICE`, `GFM_CELL_NOT_PLACED`, `GFM_TABLE_NOT_PROJECTED`, `GFM_LIST_ITEM_RUN_JOINS`, `MCID_RUN_JOINS`, `BASELINE_RUN_JOINS_ABUTTED`, `BASELINE_RUN_JOINS_SPACED`, `HEADING_LEVEL_UNRESOLVED`, `HEADING_LEVEL_UNREPRESENTABLE` |
| `locate` | `locate`, `Locations`, `Occurrence`, `OccurrencePart`, `OccurrencesWithheld`, `Searched`, `LOCATIONS_ARTIFACT_TYPE`, `LOCATIONS_SCHEMA_VERSION`, `LOCATE_RULE_V1`, `LOCATE_MAX_QUOTE_BYTES`, `LOCATE_MAX_OCCURRENCES` — **new at v2.3**. Where a string lies in a representation, and nothing else: no verdict, no score, and a string occurring nowhere is an empty answer rather than an error. The two ceilings are `ethos.grounding.v1`'s own, re-declared because this crate may not depend on the grounding crate and asserted equal by a test that can see both |
| `profile` | `Profile`, `profile_sha256`, `BackendIdentity`, `Capabilities`, `PageBudget`, `XrefRepair`, `VerifierPin`, `TableDetection`, `CMAP_DATA_VERSION`, `READING_ORDER_RULE_V0`, `READING_ORDER_RULE_V1`, `READING_ORDER_RULE_V2`, `READING_ORDER_RULE_V3`, `OBSERVATION_RULE_V1`, `TEXT_CODE_RULE_V1`, `RasterDpi`, `NOT_RUN`, `DOCX_READING_ORDER_RULE_V1`, `DOCX_TEXT_CODE_RULE_V2`, `XLSX_READING_ORDER_RULE_V1`, `XLSX_TEXT_CODE_RULE_V2`, `PPTX_READING_ORDER_RULE_V1`, `PPTX_TEXT_CODE_RULE_V2`, `ODT_READING_ORDER_RULE_V1`, `ODT_TEXT_CODE_RULE_V2`, `ODS_READING_ORDER_RULE_V1`, `ODS_TEXT_CODE_RULE_V2`, `ODP_READING_ORDER_RULE_V1`, `ODP_TEXT_CODE_RULE_V2`, `RTF_READING_ORDER_RULE_V1`, `RTF_TEXT_CODE_RULE_V1`, `EPUB_READING_ORDER_RULE_V1`, `EPUB_TEXT_CODE_RULE_V1`, `TABLE_DETECTION_V1`, `TABLE_DETECTION_V2`, `TABLE_DETECTION_V3`, `TABLE_DETECTION_V4`, `TABLE_DETECTION_V5`, `TABLE_DETECTION_V6`, `TABLE_DETECTION_UNRULED_V1`, `TABLE_DETECTION_STROKE_V1`, `TABLE_DETECTION_TAGGED_V1`, `STRUCT_TREE_RULE_V1`, `STRUCT_TREE_RULE_V2`, `HEADING_INFERENCE_RULE_V2` (**new with decision #29**), `FORM_ANNOTATION_RULE_V1` |
...
- `VerifierPin` — which verifier a run was bound to. `NotPinned` is the default and is a real
  ...
  verifier says so. `ethos-parser verify` pins the binary it spawned by version and digest.
...
`crates/ethos-parser-cli/tests/diagnostics.rs` asserts the separation.
...
## `ethos-parser-grounding` — the projection and its validator
...
## `ethos-parser-office` — the OOXML readers, the three OpenDocument readers, RTF and EPUB
...
`ethos_parser_office::read` — the entry point for every non-PDF format `ethos-parser extract` accepts —
...
| `ethos-parser extract <pdf>` | `Document::open` → `ethos_parser_pdf::extract` → `ethos_parser_pdf::to_representation` → `DocumentRepresentation::write_canonical_to`, which streams the bytes `to_canonical_bytes` returns | `library_surface.rs::extract_and_represent_are_reachable_and_canonical_from_the_library`, `library_surface.rs::write_canonical_to_writes_the_bytes_to_canonical_bytes_returns` | `grounding.rs::the_cli_path_matches_the_library` |
...
The exit-code mapping is library-owned too — `ethos_parser_pdf::exit::exit_code` for `classify` and
...
3. `cargo test -p ethos-parser-cli --test public_api` goes green, or one of the two was forgotten.
