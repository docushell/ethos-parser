...
| [`ligatures.py`](ligatures.py) | Runs whose codes decode to more characters than there are codes, beyond synthesized spaces, the state of their boxes, and each measured box's width against that run's own advance. Several representations at once print one total |
