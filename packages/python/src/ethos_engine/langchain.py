...
``engine-cli/src/mcp.rs`` rather than written afresh: MCP already decided the words, and a second
adapter with a richer sentence is a second thing to keep honest forever.

# It is not a third implementation

Every tool calls :mod:`ethos_engine` — the same functions a shell would reach through the CLI.
Nothing here spawns `engine`, and nothing here talks to MCP. If a tool returned bytes the SDK
...
``langchain-core`` is an **optional extra**, so ``import ethos_engine`` still pulls nothing:
...
try:
    ...
    raise ImportError(
        "`ethos_engine.langchain` needs langchain-core, which is an optional extra so that "
        "`import ethos_engine` pulls nothing. Install it with:\n\n"
        "    pip install 'ethos-engine[langchain]'\n\n"
