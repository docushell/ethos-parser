...
"""The Python SDK — a thin surface over the ``ethos-parser`` CLI (v1.2-S2).
...
:class:`EngineNotFound`, not a reason to go looking for some other build — then ``ethos-parser`` on
...
def node_get(representation, node_id):
    ...
    **The handle law, made mechanical**, and the one function here with no subcommand behind it.
    ``ethos-parser node-get`` does not exist and this slice does not add it — MCP already carries the
...
def _binary():
    ...
    found = shutil.which("ethos-parser")
    if found:
        return found
