...
"""c14n v1 in Python — the same canonical serialization ``ethos-parser-core/src/c14n.rs`` writes.

**Why this file exists at all.** :func:`ethos_parser.node_get` has no CLI subcommand behind it,
...
``tests/test_cli_surface.py`` re-canonicalizes what ``ethos-parser extract`` printed and compares the
